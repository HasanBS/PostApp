...
extension ContextExtension on BuildContext {
  ...
  MaterialColor oneTo10Color(int i) => Colors.primaries[i];
...
extension DoubleNumberExtension on BuildContext {
  String doubleToString(double val) =>
      val % 1 == 0 ? (val.round()).toString() : val.toString();
}
...
extension PaddingExtension on BuildContext {
  ...
  EdgeInsets get horizontalPaddingLow =>
      EdgeInsets.symmetric(horizontal: lowValueW);
  EdgeInsets get horizontalPaddingNormal =>
      EdgeInsets.symmetric(horizontal: normalValueW);
  EdgeInsets get horizontalPaddingMedium =>
      EdgeInsets.symmetric(horizontal: mediumValueW);
  EdgeInsets get horizontalPaddingHigh =>
      EdgeInsets.symmetric(horizontal: highValueW);

  EdgeInsets get verticalPaddingLow =>
      EdgeInsets.symmetric(vertical: lowValueH);

  EdgeInsets get verticalPaddingNormal =>
      EdgeInsets.symmetric(vertical: normalValueH);
  EdgeInsets get verticalPaddingMedium =>
      EdgeInsets.symmetric(vertical: mediumValueH);
  EdgeInsets get verticalPaddingHigh =>
      EdgeInsets.symmetric(vertical: highValueH);
}
...
extension BorderExtension on BuildContext {
  ...
  RoundedRectangleBorder get roundedRectangleBorderNormal =>
      RoundedRectangleBorder(
          borderRadius:
              BorderRadius.vertical(top: Radius.circular(normalValueH)));

  RoundedRectangleBorder get roundedRectangleBorderMedium =>
      RoundedRectangleBorder(
          borderRadius:
              BorderRadius.vertical(top: Radius.circular(mediumValueH)));

  RoundedRectangleBorder get roundedRectangleBorderHigh =>
      RoundedRectangleBorder(
          borderRadius:
              BorderRadius.vertical(top: Radius.circular(highValueH)));
}
...
extension NavigationExtension on BuildContext {
  ...
  Future<T?> navigateToReset<T>(String path, {Object? data}) async {
    return navigation.pushNamedAndRemoveUntil(path, (route) => false,
        arguments: data);
  }
}
